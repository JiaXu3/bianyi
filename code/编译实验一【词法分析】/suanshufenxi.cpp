...
void suanshufenxi::scanOneLine(){//词法分析一行
	...
	while (i != line.length()){
		...
		case '>':
			...
			ccount++; break;
		case '+':
			buf[ccount].num = jia; ccount++; break;
		case '-':
			buf[ccount].num = jian; ccount++; break;
		case '*':
			buf[ccount].num = chen; ccount++; break;
		case '/':
			buf[ccount].num = chu; ccount++; break;
		case '=':
			buf[ccount].num = deng; ccount++; break;
		case '!':
			readch();
			if (ch == '='){
		...
		case ',':
			buf[ccount].num = douhao; ccount++; break;
		case ';':
			buf[ccount].num = fenhao; ccount++; break;
		case '{':
			buf[ccount].num = dal; ccount++; break;
		case '}':
			buf[ccount].num = dar; ccount++; break;
		case '(':
			buf[ccount].num = xiaol; ccount++; break;
		case ')':
			buf[ccount].num = xiaor; ccount++; break;
		case '#':
			buf[ccount].num = jinhao; ccount++; break;
		default: buf[ccount].num = err; buf[ccount].pos = count_err; errs[count_err] = ch; errsLine[count_err] = nowline; count_err++; ccount++; break;
		}
		ch = line[++i];
	}
...
void suanshufenxi::reset(){//重置语法分析栈
	...
	sstack[sp].danci.num = jinhao;
	sstack[sp].danci.pos = 0;
...
void suanshufenxi::show3(int c){//输出计算到当前这一步的计算结果
	...
	for (int p = 0; p <sl; p++){
		...
		else if (sstack[p].danci.num == kongzi){//空字
			sss[p].b = "kong";
		}
		else{//终结符
...
int suanshufenxi::conv(int c, ci tem){//根据输入符号返回在LR分析表中的横向序号
	if (c == 2 || c == 13){
		if (tem.num == jia)
			return 1;
...
void suanshufenxi::lrparse(){//语法分析
	...
	/*if (ii == (ccount - 1))
	return;*/
	lr = action22[sstack[sp].status][conv(sstack[sp].status, buf[ii])];
	if ((lr >= 0) && (lr < 18)){
		if (lr == 11){
			sstack[++sp].status = lr;

			sstack[sp].danci.num = kongzi;
			sstack[sp].danci.pos = 0;
		...
		case 101://E->TE'
			sp = sp - 2;
			lrt = action22[sstack[sp].status][E2];
			++sp;
			sstack[sp].status = lrt;
			...
			if (sstack[sp + 1].c == kongzi){
				...
				sstack[sp].c = ttt;
				if (buf[ii].num != xiaor)//如果是小括号里只有一个数字这种情况就不输出
					show3(E2);
			}

			sstack[sp].danci.num = -1;
			sstack[sp].danci.pos = 0;
		...
		case 102://E'->+TE'
			sp = sp - 3;
			lrt = action22[sstack[sp].status][E_];
			++sp;
			sstack[sp].status = lrt;
		...
		case 104://T->FT'
			sp = sp - 2;
			lrt = action22[sstack[sp].status][T];
			++sp;
			sstack[sp].status = lrt;
		...
		case 105://T'->*FT'
			sp = sp - 3;
			lrt = action22[sstack[sp].status][T_];
			++sp;
			sstack[sp].status = lrt;
		...
		case 107://F->(E)
			sp = sp - 3;
			lrt = action22[sstack[sp].status][F];
			++sp;
			sstack[sp].status = lrt;
		...
		case 108://F->i
			...
			sstack[sp].c = sstack[sp].danci.pos;
