...
void analyze::stYufa(){//开始进行语法分析
	...
	//out << endl;
	//cout << endl;
	ii = 0;//当前要分析的符号在词法分析结果(buf表)中的位置
	ret2 = 0;//程序语句退出标记
	reset();//重置语法分析栈
	lrparse();//语法分析
}
...
void analyze::identifier(){//识别字符串是标识符还是保留字
	...
	do{
		spelling.append(&ch, 1); //将串存到spelling
		readch();
	} while ((ch >= 'a') && (ch <= 'z') || (ch >= 'A') && (ch <= 'Z') || (ch >= '0') && (ch <= '9'));
	backch();
	while ((ss == 0) && (iii < nOFkeepwords)){ //读取的串一个个与保留字比较
		if (spelling == kw[iii].name)
			ss = 1;
		iii++;
	}
	if (ss == 1)
		buf[ccount].num = kw[iii - 1].value;
	else
	{
		buf[ccount].num = biaoshifu;
		j = findd(spelling);
		if (j == -1){
...
void analyze::scanOneLine(){//词法分析一行
	...
	while (i != line.length()){
		...
		case '<':
			readch();
			if (ch == '='){
				buf[ccount].num = xiaodeng;
			}
			else{
				buf[ccount].num = smaller;
				backch();
			}
			buf[ccount].linecount = nowline;
			ccount++; break;
		case '>':
			readch();
			if (ch == '='){
				buf[ccount].num = dadeng;
			}
			else{
				buf[ccount].num = bigger;
				backch();
			}
			buf[ccount].linecount = nowline;
			ccount++; break;
		...
		case '&':
			...
			if (ch == '&'){
				...
				backch();
				buf[ccount].num = err; buf[ccount].pos = count_err; errs[count_err] = ch; count_err++;
		...
		case ',':
			buf[ccount].num = douhao;
			buf[ccount].linecount = nowline; ccount++; break;
		case ';':
			buf[ccount].num = fenhao;
			buf[ccount].linecount = nowline; ccount++; break;
		case '{':
			buf[ccount].num = dal;
			buf[ccount].linecount = nowline; ccount++; break;
		case '}':
			...
			buf[ccount].linecount = nowline; ccount++; break;
		case '(':
			buf[ccount].num = xiaol;
			buf[ccount].linecount = nowline; 
			ccount++; break;
		case ')':
			buf[ccount].num = xiaor;
			buf[ccount].linecount = nowline; ccount++; break;
		case '#':
			buf[ccount].num = jinhao;
			buf[ccount].linecount = nowline; ccount++; break;
		default: buf[ccount].num = err;
...
void analyze::show(){//输出词法分析的结果
	...
	for (int q = 0; q < ccount-1; q++){
		if (buf[q].num >= 0 && buf[q].num < nOFkeepwords){//保留字
			cout << "\n(" << buf[q].num << ",\"" << kw[buf[q].num].name << "\")";
		}
		else if (buf[q].num == 20){//标识符
			cout << "\n(" << buf[q].num << ",\"" << tab_ch[buf[q].pos] << "\")";
		}
		else if (buf[q].num == 21){//数字
			cout << "\n(" << buf[q].num << ",\"" << buf[q].pos << "\")";
		}
		else if (buf[q].num >= 22 && buf[q].num < 28){//其他
			cout << "\n(" << buf[q].num << ",\"" << others[buf[q].num - 22].name << "\")";
		}
		else if (buf[q].num >= 50 && buf[q].num < 70){//运算符
			cout << "\n(" << buf[q].num << ",\"" << yunsuan[buf[q].num - 50].name << "\")";
		}
		else if (buf[q].num >= 70 && buf[q].num <= 80){//界符
			cout << "\n(" << buf[q].num << ",\"" << border[buf[q].num - 70].name << "\")";
		}
		else if (buf[q].num == err){
...
/*
			...
			return yunsuan[buf[q].num - 50].name;
		}
		else if (buf[q].num >= 70 && buf[q].num <= 80){//界符
			return border[buf[q].num - 70].name;
		}
...
void analyze::showFuhao(){
	for (int i = 0; i <= sp; i++)
		cout << changeStackB(stack[i].danci.num, stack[i].danci.zhongjie) << " ";
}
...
void analyze::reset(){//重置语法分析栈
	sp = 0;
	stack[sp].status = 0;//状态栈初始为s0

	stack[sp].danci.num=jinhao;//符号栈先压入#号
	stack[sp].danci.pos=0;
...
void analyze::reset2(){//重置语法分析栈
	...
	sstack[ssp].danci.num = jinhao;
	sstack[ssp].danci.pos = 0;
...
void analyze::show2(int st, int en){//输出词法分析结果中从位置ista到iend的元素
	
	{
		for (int q = st; q <= en; q++){
			if (buf[q].num >= 0 && buf[q].num < nOFkeepwords){//保留字
				...
				out << others[buf[q].num - 22].name << " ";
			}
			else if (buf[q].num >= 50 && buf[q].num < 70){//运算符
				out << yunsuan[buf[q].num - 50].name << " ";
			}
			else if (buf[q].num >= 70 && buf[q].num <= 80){//界符
				out << border[buf[q].num - 70].name << " ";
			}
			else if (buf[q].num == err){
				out << errs[buf[q].pos] << " ";
			}
	...
	for (int q = st; q <= en; q++){
		if (buf[q].num >= 0 && buf[q].num < nOFkeepwords){//保留字
			...
			cout << others[buf[q].num - 22].name << " ";
		}
		else if (buf[q].num >= 50 && buf[q].num < 70){//运算符
			cout << yunsuan[buf[q].num - 50].name << " ";
		}
		else if (buf[q].num >= 70 && buf[q].num <= 80){//界符
			cout << border[buf[q].num - 70].name << " ";
		}
		else if (buf[q].num == err){
			cout << errs[buf[q].pos] << " ";
		}
...
void analyze::stsend(){//语义分析时计算当前表达式的开始位置和结束位置
	...
	iend = ista;
	while ((buf[iend].num != douhao) && (buf[iend].num != fenhao) && (buf[iend].num != jinhao)&&(iend<ccount-1))
	{
...
void analyze::show3(){//输出计算到当前这一步的计算结果
	...
	for (int p = 0; p <sl; p++){
		if (p == 0)
			sss[p].b = "=";
		else if (sstack[p].danci.num == -1){//非终结符
			sss[p].a = sstack[p].c;
			sss[p].b = "-";
		}
		else{//终结符
			sss[p].b = finds(sstack[p].danci.num);
		}
	}
...
int analyze::conv2(ci tem){//根据输入符号返回在LR分析表中的横向序号
	switch (tem.num){
	case changshu:case biaoshifu:return 0;
	case jia:return 1;
	case chen:return 2;
	case xiaol:return 3;
	case xiaor:return 4;
...
int analyze::conv3(ci tem){//根据输入符号返回在布尔表达式LR分析表中的横向序号
	switch (tem.num){
	case changshu:case biaoshifu:return 0;
...
int analyze::conv(ci tem){//根据输入符号返回在程序语句LR分析表中的横向序号
	switch (tem.num){
	case s_if:return 0;
	...
	default:if (tem.num == xiaol ) return 5;else  return 3;
	}
}
...
int analyze::lrparse2(){//算术表达式的分析
	...
	lr = action2[sstack[ssp].status][conv2(buf2[jj])];
	if ((lr >= 0) && (lr < 10)){
			sstack[++ssp].status = lr; // sstack[100]算术或布尔表达式语法分析栈
			sstack[ssp].danci = buf2[jj++];
			/*if (jj > iend){
				lr = -20;
				goto erro;
			}*/
...
/*	else */if ((lr > 100) && (lr < 109)){
		...
		//	break;
		case 101://E->E+E
			Et.pos = newtemp(); //产生一个新的临时变量 //Et产生式左部
			emit("+", sstack[ssp-2].danci, sstack[ssp].danci, Et.pos+100);//产生一个四元式
			
			ssp = ssp - 3; //算数分析的栈顶指针-3
			lrt = action2[sstack[ssp].status][E]; //用E规约之后，用E填补与E对应的状态
			++ssp;
			sstack[ssp].status = lrt;
		...
		case 102://E->E*E
			Et.pos = newtemp();
			emit("*", sstack[ssp - 2].danci, sstack[ssp].danci, Et.pos + 100);
			ssp = ssp - 3;
			lrt = action2[sstack[ssp].status][E];
			++ssp;
			sstack[ssp].status = lrt;
		...
		case 103://E->(E)
			...
			//
			sstack[ssp].danci = sstack[ssp+1].danci;
			...
			break;
		case 104://E->i
			Et.pos = sstack[ssp].danci.pos;
			ssp = ssp - 1;
			lrt = action2[sstack[ssp].status][E];
			++ssp;
			sstack[ssp].status = lrt;

			sstack[ssp].c = sstack[ssp].danci.pos;
		...
		sstack[ssp].danci.pos = Et.pos;
		lrparse2();
	}
if (lr == acc&&ret == 0){
	//iend = jj;
	emit("=", sstack[ssp].danci, downline, buf2[0].pos);
...
int analyze::emit(string op1, ci arg11, ci arg22, int result1){//产生一个四元式
	fexp[nxq].op = op1;
	fexp[nxq].arg1.num = arg11.num;
	fexp[nxq].arg1.pos = arg11.pos;
	fexp[nxq].arg1.zhongjie = arg11.zhongjie;
	fexp[nxq].arg2.num = arg22.num;
...
int analyze::lrparse3(){//布尔表达式的分析
	if (ret == 1)
		return 1;
	lr = action3[sstack[ssp].status][conv3(buf2[jj])];
	if ((lr >= 0) && (lr < 16)){
		sstack[++ssp].status = lr;
		sstack[ssp].danci = buf2[jj++];
		/*if (jj > iend){
			lr = -20;
			goto erro;
	...
	/*else*/ if ((lr > 100) && (lr < 109)){
		...
		//	break;
		case 101://B->i
			ntab3[label].tc = nxq;     //布尔表达式的两个地址
			ntab3[label].fc = nxq+1;
			emit("jnz", sstack[ssp].danci, downline, 0);
			emit("j", downline, downline, 0);

			ssp = ssp - 1;
			label++;

			lrt = action3[sstack[ssp].status][B];
			++ssp;
			sstack[ssp].status = lrt;
		...
		case 102://B->i rop i
			ntab3[label].tc = nxq;
			ntab3[label].fc = nxq + 1;
			switch (sstack[ssp-1].danci.num){
			case xiaodeng:
				emit("j<=", sstack[ssp - 2].danci, sstack[ssp].danci, 0);
				break;
			case smaller:
				emit("j<", sstack[ssp - 2].danci, sstack[ssp].danci, 0);
				break;
		...
		case 105://A->B and
			...
			label++;
			ssp = ssp - 2;
			lrt = action3[sstack[ssp].status][A];
			++ssp;
			sstack[ssp].status = lrt;
		...
		case 107://O->B or
			...
			ssp = ssp - 2;
			lrt = action3[sstack[ssp].status][O];
			++ssp;
			sstack[ssp].status = lrt;
	...
	erro:
		...
		case -24:er = "or出现在最左边\n\n"; break;
		case -25:er = "比较符号" + finds(buf2[jj].num)+"没有左操作数\n"; break;
		default:er = " \n\n"; break;
		}
...
int analyze::test(int value){//判断当前输入字符是程序语句的话返回0，否则返回1（布尔表达式或算术表达式）
	switch (value){
	case biaoshifu:case changshu:case jia:case chen:
...
int analyze::lrparse(){//程序语句的分析
	...
	if ((lr >= 0) && (lr <= 18))
	{
		showZhan(lr);
		{
			...
			if (test(buf[ii].num))
			{//判断当前输入字符是程序语句的话返回0，否则返回1（布尔表达式或算术表达式）
				
				if (stack[sp].danci.num == s_while)//sign=1表明是算术表达式语句，sign=2表明是循环语句，sign=3表明是控制语句
					sign = 2;
				else
				{
					if (stack[sp].danci.num == s_if)
						sign = 3;
					else
						sign = 1;
				}
				do{
					buf2[jj].pos = buf[ii].pos;//buf2保存算术或布尔表达式的输入字符串
					buf2[jj].num = buf[ii].num;
					buf2[jj].linecount = buf[ii].linecount;
					ii++;
					jj++;

				} while (test(buf[ii].num));//检测到布尔表达式或算术表达式之后，将表达式整体存入buf2中
				buf2[jj].num = fenhao;
				//jj = 0;
				if (sign == 1){//算术表达式
					jj = 2;
					subline = buf[ii].linecount; //分析程序的错误
					reset2();
					ret = 0;
					lrparse2();//算术表达式的分析
					ii--;
				...
				if ((sign == 2) || (sign == 3)){//布尔表达式处理
					pointmark++;
					labelmark[pointmark].nxq1 = nxq;//保存if或while的四元式序号，位置，从100开始
					jj = 0;
					subline = buf2[0].linecount; //分析程序的错误
					reset2();
					ret = 0;
					lrparse3();//布尔表达式的分析
					labelmark[pointmark].turec1 = ntab3[label - 1].tc; //保存if或while正确或错误后指向哪
					labelmark[pointmark].faultc1 = ntab3[label - 1].fc;
					backpatch(labelmark[pointmark].turec1, nxq);
					ii--;
					buf[ii].num = buer;
				}
				lr = action[stack[sp].status][conv(buf[ii])];
			}

		}
		stack[++sp].status = lr; //程序语句的移进操作
		stack[sp].danci = buf[ii++];
		/*if (ii > iend){
			lr = -20;
			goto erro;
		}*/
	...
	/*else*/ if ((lr > 100) && (lr < 107)){
		...
		case 101://S->if e S else S //S->if e then S else S
			...
			if (stack[sp - 1].danci.num == s_if){
				emit("j", downline, downline, 0);//emit产生一个四元式
				backpatch(labelmark[pointmark].faultc1, nxq); // 返填函数
				pointtemp++;
				labeltemp[pointtemp] = nxq - 1;
			}
			
			if (stack[sp - 1].danci.num == s_while){
				emit("j", downline, downline, labelmark[pointmark].nxq1);
				backpatch(labelmark[pointmark].faultc1, nxq);
			}

			lrt = action[stack[sp].status][S];
			++sp;
			stack[sp].status = lrt;

			stack[sp].danci.num =S;//非终结符
			stack[sp].danci.zhongjie = -2;
			stack[sp].danci.pos = 0;
			//showZhan(lrt);
			//stack[sp].b = { -1, 0 };
		...
		case 102://S->while e S //while e do S
			...
			if (stack[sp - 1].danci.num == s_if){
				emit("j", downline, downline, 0);
				fexp[labelmark[pointmark].faultc1].result = nxq;
				pointtemp++;
				labeltemp[pointtemp] = nxq - 1;
			}
...
void analyze::show4exp(){//输出四元式
	...
	for (k; k < nxq - 1; k++){
		...
		{
			if (fexp[k].arg1.num == biaoshifu)
				cout << tab_ch[fexp[k].arg1.pos] << "	,	";			
			else{
				if (fexp[k].arg1.num == changshu)
					cout << fexp[k].arg1.pos << "	,	";
				else
					cout << "_	,	";
			}
		...
		if (fexp[k].arg2.zhongjie == -2)
			...
			if (fexp[k].arg2.num == biaoshifu)
				cout << tab_ch[fexp[k].arg2.pos] << "	,	";
			
			else{
				if (fexp[k].arg2.num == changshu)
					cout << fexp[k].arg2.pos << "	,	";
				else
					cout << "_	,	";
			}
