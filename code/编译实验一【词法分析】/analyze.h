...
class analyze
{
	...
	ci buf[3000];//词法分析表，保存词法分析的结果
	ci buf2[300];//保存算术或布尔表达式的输入字符串
...
public:
	...
	int conv(ci tem);//根据输入符号返回在程序语句LR分析表中的横向序号
	int conv2(ci tem);//根据输入符号返回在算术表达式LR分析表中的横向序号
	int conv3(ci tem);//根据输入符号返回在布尔表达式LR分析表中的横向序号
	...
	int emit(string op1, ci arg1, ci arg2, int result1);//产生一个四元式
